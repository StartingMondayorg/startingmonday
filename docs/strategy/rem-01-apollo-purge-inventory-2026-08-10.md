...
- Verified: admin status route (`/api/admin/edgar-status`) now includes `compatibilitySunset` readiness fields (`hitCount`, `hitBudget`, `sunsetReady`, `lastSeenAt`) and `status.compatRouteUsage` sourced from the compatibility telemetry key.
