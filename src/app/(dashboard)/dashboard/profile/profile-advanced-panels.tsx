...
export function ProfileAdvancedPanels(props: Props) {
  ...
  return (
    <>
      <Accordion className="rounded border border-slate-200 px-4 py-3">
        <AccordionItem id="section-linkedin-upload" value="linkedin-upload">
          <AccordionTrigger className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500">
            LinkedIn profile upload
          </AccordionTrigger>
          <AccordionContent>
            <Label className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">
      ...
      <div>
        <Label htmlFor="beyond_resume" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Beyond the resume</Label>
      ...
      <Accordion className="rounded border border-slate-200 px-4 py-3">
        <AccordionItem value="interview-stories">
          <AccordionTrigger className="text-[11px] font-bold tracking-[0.08em] uppercase text-orange-500">Interview stories</AccordionTrigger>
          <AccordionContent>
      ...
      <Accordion className="rounded border border-slate-200 px-4 py-3">
        <AccordionItem value="role-context">
          <AccordionTrigger className="text-[11px] font-bold tracking-[0.08em] uppercase text-orange-500">Role context</AccordionTrigger>
          <AccordionContent className="flex flex-col gap-6">
            <p className="text-[12px] text-slate-400">Add details that do not fit in your resume.</p>
            {props.roleType === 'ciso' && (
              <div className="flex flex-col gap-4">
                <div>
                  <Label className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Security frameworks implemented</Label>
                ...
                <div>
                  <Label htmlFor="board_security_maturity" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Board security maturity</Label>
            ...
            {props.roleType === 'cpo' && (
              <div className="flex flex-col gap-4">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-2">Product experience type</p>
                  <RadioGroup name="product_type_exp" defaultValue={props.productTypeExp} className="flex flex-row gap-4 w-auto">
                    {(['B2C', 'B2B', 'Both'] as const).map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><RadioGroupItem value={opt} /><span className="text-[13px] text-slate-700">{opt}</span></label>)}
                  </RadioGroup>
                </div>
                <div>
                  <Label htmlFor="product_achievement" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Key product achievement</Label>
                ...
                <div>
                  <Label htmlFor="product_metric" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Primary metric moved</Label>
            ...
            {props.roleType === 'coo' && (
              <div className="flex flex-col gap-4">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-2">Mandate type(s) sought</p>
                  <div className="flex flex-col gap-2">
                    {(['Scaling', 'Turnaround', 'Post-M&A integration', 'Professionalization'] as const).map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><Checkbox name="coo_mandate_types" value={opt} defaultChecked={props.cooMandateTypes.includes(opt)} /><span className="text-[13px] text-slate-700">{opt}</span></label>)}
                  </div>
                </div>
                <div>
                  <Label htmlFor="coo_ceo_partnership" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">CEO partnership model</Label>
            ...
            {props.roleType === 'cto' && (
              <div className="flex flex-col gap-4">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-2">Technical flavor</p>
                  <div className="flex flex-col gap-2">
                    {(['Infrastructure', 'Product engineering', 'Platform', 'AI and ML'] as const).map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><Checkbox name="cto_technical_flavor" value={opt} defaultChecked={props.ctoTechnicalFlavor.includes(opt)} /><span className="text-[13px] text-slate-700">{opt}</span></label>)}
                  </div>
                </div>
                <div>
                  <Label htmlFor="cto_architecture_decision" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Key architectural decision</Label>
            ...
            {props.roleType === 'cdo_data' && (
              <div className="flex flex-col gap-4">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-2">Data mandate orientation</p>
                  <RadioGroup name="data_maturity_orientation" defaultValue={props.dataMaturityOrientation} className="flex flex-row gap-4 w-auto">
                    {(['Governance-first', 'Products-first'] as const).map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><RadioGroupItem value={opt} /><span className="text-[13px] text-slate-700">{opt}</span></label>)}
                  </RadioGroup>
                </div>
                <div>
                  <Label htmlFor="data_platform_built" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Data platform built</Label>
            ...
            {props.roleType === 'cdo_digital' && (
              <div className="flex flex-col gap-4">
                <div>
                  <p className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-2">Professional background</p>
                  <div className="flex flex-col gap-2">
                    <RadioGroup name="digital_background_type" defaultValue={props.digitalBackgroundType} className="flex flex-col gap-2 w-auto">
                      {(['Consulting', 'Operations', 'Marketing', 'Technology'] as const).map(opt => <label key={opt} className="flex items-center gap-2 cursor-pointer"><RadioGroupItem value={opt} /><span className="text-[13px] text-slate-700">{opt}</span></label>)}
                    </RadioGroup>
                  </div>
                </div>
                <div>
                  <Label htmlFor="digital_transformation_delivered" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Business transformation delivered</Label>
      ...
      <Accordion className="bg-white border border-slate-200 rounded p-6 max-w-xl mt-6">
        <AccordionItem id="section-data-privacy" value="data-privacy">
          <AccordionTrigger className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500">Data and privacy</AccordionTrigger>
          <AccordionContent>
            <section id="section-briefing" className="mb-5">
              ...
              <Label htmlFor="briefing_time" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Daily briefing time</Label>
              ...
              <div id="briefing-email" className="mt-4">
                <Label htmlFor="briefing_email" className="text-[11px] font-bold tracking-[0.08em] uppercase text-slate-500 mb-1.5">Briefing delivery email</Label>
