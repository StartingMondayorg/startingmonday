      ...
      <header className="bg-slate-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/dashboard/admin" className="text-[13px] font-semibold text-slate-400 hover:text-slate-200">Admin</Link>
          </div>
        </div>
      </header>
...
{/* Prospect header */}
            ...
            <div className="text-right shrink-0">
              {prospect.estimated_arr && (
                <div className="text-[20px] font-bold text-slate-900">
                  ...
                  <span className="text-[13px] font-normal text-slate-400 ml-1">ARR</span>
                </div>
              )}
                ...
                {(contacts ?? []).map(c => (
                  <div key={c.id} className="px-5 py-3.5 flex items-start justify-between gap-4">
                    <div>
                      ...
                      {c.title && <div className="text-[13px] text-slate-500">{c.title}</div>}
                      <div className="flex items-center gap-3 mt-0.5">
                        {c.email && <a href={`mailto:${c.email}`} className="text-[13px] text-slate-400 hover:text-slate-700">{c.email}</a>}
                        {c.linkedin_url && <a href={c.linkedin_url} target="_blank" rel="noreferrer" className="text-[13px] text-slate-400 hover:text-slate-700">LinkedIn</a>}
                      </div>
                      {c.notes && <p className="text-[13px] text-slate-400 mt-1">{c.notes}</p>}
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <div key={a.id} className="px-5 py-3.5">
                    ...
                    {a.next_action && (
                      <div className="mt-1.5 flex items-center gap-2">
                        <span className="text-[13px] font-semibold text-slate-400 uppercase tracking-[0.06em]">Next:</span>
                        <span className="text-[13px] text-slate-700">{a.next_action}</span>
                        {a.next_action_due && <span className="text-[13px] text-slate-400">by {a.next_action_due}</span>}
                      </div>
                    )}
                  </div>
                ))}
        ...
        <section>
          <h2 className="text-[13px] font-bold tracking-[0.1em] uppercase text-slate-400 mb-3">Leave-behind materials</h2>
          <MaterialClient
                ...
                <Card key={m.id} className="p-0">
                  <div className="px-5 py-3 flex items-center justify-between border-b border-slate-100">
                    <div>
                      ...
                      <span className="text-[13px] text-slate-400 ml-3">{new Date(m.created_at).toLocaleDateString()}</span>
                    </div>
