  ...
  return (
    <div className="rounded border border-border bg-card p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <input disabled={!enabled || companies.length >= 10} value={draft.company_name} onChange={(event) => setDraft({ ...draft, company_name: event.target.value })} placeholder="Company name" className="h-9 rounded border border-border bg-background px-3 text-[12px] outline-none focus:border-primary disabled:bg-muted" />
          <input disabled={!enabled || companies.length >= 10} value={draft.company_key} onChange={(event) => setDraft({ ...draft, company_key: event.target.value })} placeholder="Stable company key" className="h-9 rounded border border-border bg-background px-3 text-[12px] outline-none focus:border-primary disabled:bg-muted" />
          <input disabled={!enabled || companies.length >= 10} value={draft.career_page_url} onChange={(event) => setDraft({ ...draft, career_page_url: event.target.value })} placeholder="Career page URL (optional)" className="h-9 rounded border border-border bg-background px-3 text-[12px] outline-none focus:border-primary disabled:bg-muted" />
          <input disabled={!enabled || companies.length >= 10} value={draft.target_role_lane} onChange={(event) => setDraft({ ...draft, target_role_lane: event.target.value })} placeholder="Target role lane (optional)" className="h-9 rounded border border-border bg-background px-3 text-[12px] outline-none focus:border-primary disabled:bg-muted" />
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <button type="button" disabled={!enabled || companies.length >= 10} onClick={addCompany} className="rounded border border-border bg-card px-3 py-2 text-[12px] font-semibold text-foreground disabled:opacity-50">Add company</button>
