    ...
    <section className="rounded border border-border bg-card p-4 sm:p-5 mb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {products.slice(0, 2).map((product) => {
          // Account routes are auth-gated: their RSC prefetch never settles, so the
          // page never reaches network idle. Skip prefetch for them.
          const isAccountRoute = product.ctaHref.startsWith('/settings/')

          return (
            <TrackLink
              key={product.slug}
              href={product.ctaHref}
              rel={isAccountRoute ? 'nofollow' : undefined}
              prefetch={isAccountRoute ? false : undefined}
