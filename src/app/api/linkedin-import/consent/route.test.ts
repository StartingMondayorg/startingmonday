import { describe, expect, it, vi } from 'vitest'
import { NextRequest } from 'next/server'

const state = vi.hoisted(() => ({
  isRelationshipNetworkMatchingEnabled: vi.fn(),
  requireAuth: vi.fn(),
}))

vi.mock('@/lib/feature-flags', () => ({
  isRelationshipNetworkMatchingEnabled: state.isRelationshipNetworkMatchingEnabled,
}))

vi.mock('@/lib/require-auth', () => ({
  requireAuth: state.requireAuth,
}))

import { POST, DELETE } from './route'

describe('src/app/api/linkedin-import/consent/route.ts', () => {
  it('returns 403 when relationship network matching is disabled', async () => {
    state.isRelationshipNetworkMatchingEnabled.mockReturnValue(false)

    const postResponse = await POST(new NextRequest('https://startingmonday.app/api/linkedin-import/consent', {
      method: 'POST',
      headers: { 'content-type': 'multipart/form-data; boundary=test' },
      body: new FormData(),
    }))
    const deleteResponse = await DELETE(new NextRequest('https://startingmonday.app/api/linkedin-import/consent?consent_id=test', {
      method: 'DELETE',
    }))

    expect(postResponse.status).toBe(403)
    await expect(postResponse.json()).resolves.toEqual({ error: 'Relationship network matching is currently disabled.' })
    expect(deleteResponse.status).toBe(403)
    await expect(deleteResponse.json()).resolves.toEqual({ error: 'Relationship network matching is currently disabled.' })
    expect(state.requireAuth).not.toHaveBeenCalled()
  })
})
