  ...
  return (
    <article className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-5 shadow-[0_18px_50px_rgba(15,23,42,0.24)] backdrop-blur-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-200">Live role lead {index}</p>
          <h3 className="text-[20px] font-semibold leading-tight text-white">{company}</h3>
        </div>
      ...
      <p className="mb-2 text-[14px] font-medium text-slate-100">{title}</p>
      <p className="mb-3 text-[12px] text-slate-300">Likely opening window: {window}</p>
      <p className="text-[13px] leading-relaxed text-slate-200">
        <span className="font-semibold text-white">Why now:</span> {whyNow}
      </p>
    </article>
  )
}
    ...
    <div className="min-h-screen bg-slate-950 font-sans text-slate-100">
      ...
      <nav className="sticky top-0 z-20 border-b border-white/8 bg-slate-950/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-4 sm:gap-5">
            <Link href="/coaches" className="text-[13px] text-slate-300 transition-colors hover:text-white">Coaches</Link>
            <Link href="/demo/cio" className="text-[13px] text-slate-300 transition-colors hover:text-white">CIO demo</Link>
            <Link href="/signup?from=demo" className="rounded-full bg-orange-500 px-4 py-1.5 text-[13px] font-semibold text-slate-950 transition-colors hover:bg-orange-600">
              Start free trial
            </Link>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        <section className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
          <div>
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-orange-300">Public coach demo</p>
            <h1 className="mb-5 max-w-3xl font-serif text-[2.6rem] leading-[1.02] tracking-tight text-white sm:text-[3.8rem]">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#scanner-leads" className="rounded-full bg-white px-5 py-2.5 text-[13px] font-semibold text-slate-950 transition-colors hover:bg-slate-100">
                See the three live leads
              </a>
              <a href="#why-these-firms" className="rounded-full border border-white/15 px-5 py-2.5 text-[13px] font-semibold text-white transition-colors hover:border-white/30">
                Why these firms
              </a>
            </div>
          </div>
          ...
          <aside className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.24)] backdrop-blur-sm">
            ...
            <div className="space-y-3 text-[13px] leading-relaxed text-slate-200">
              ...
              <p><span className="font-semibold text-white">Use case:</span> send this page before or after a session to keep the conversation anchored in actual market movement.</p>
            </div>
          </aside>
        </section>

        <section className="mt-10 rounded-[1.75rem] border border-emerald-300/20 bg-emerald-300/10 p-5 sm:p-6">
          <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-200">Why it matters</p>
        ...
        <section id="scanner-leads" className="mt-10">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-300">Starting Monday scanner</p>
              <h2 className="text-[24px] font-bold leading-tight text-white sm:text-[28px]">Three live role leads for the federal IT transition lane</h2>
            </div>
          </div>
        ...
        <section id="why-these-firms" className="mt-10 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.24)] backdrop-blur-sm">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-200">Why these firms</p>
          <h2 className="mb-3 text-[22px] font-bold leading-tight text-white sm:text-[26px]">The shortlist is built around transferability, not just company name.</h2>
              ...
              <div className="space-y-3">
                {EXCLUDED_EVALUATION.map((item) => (
                  <article key={item.firm} className="rounded-2xl border border-white/8 bg-slate-950/35 px-4 py-3">
                    <p className="mb-1 text-[13px] font-semibold text-white">{item.firm}</p>
        ...
        <section className="mt-10 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.24)] backdrop-blur-sm">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-200">Title translation</p>
          <div className="flex flex-wrap gap-2">
            {TITLE_TRANSLATION.map((item) => (
              <span key={item} className="rounded-full border border-white/10 bg-slate-950/60 px-3 py-1.5 text-[12px] text-slate-100">{item}</span>
            ))}
          </div>
          <p className="mt-4 max-w-3xl text-[13px] leading-relaxed text-slate-300">
        ...
        <section className="mt-10 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_18px_50px_rgba(15,23,42,0.24)] backdrop-blur-sm">
          <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.16em] text-orange-200">Send this link</p>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Link href="/demo/federal-it-transition" className="rounded-full bg-orange-500 px-5 py-2.5 text-[13px] font-semibold text-slate-950 transition-colors hover:bg-orange-600">
              Open private working version
            </Link>
            <Link href="/coaches" className="rounded-full border border-white/15 px-5 py-2.5 text-[13px] font-semibold text-white transition-colors hover:border-white/30">
              Back to coaches
            </Link>
          </div>
