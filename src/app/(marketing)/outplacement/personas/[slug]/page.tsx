...
export function generateStaticParams() {
  return OUTPLACEMENT_PERSONAS.map((persona) => ({ slug: persona.slug }))
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { slug } = await params
  const persona = OUTPLACEMENT_PERSONAS.find((item) => item.slug === slug)
  if (!persona) return {}
  return {
    title: `${persona.label} | Outplacement Buyer Role`,
    description: persona.summary,
    alternates: { canonical: `https://startingmonday.app/outplacement/personas/${persona.slug}` },
  }
}

export default async function OutplacementPersonaDetailPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params
  const persona = OUTPLACEMENT_PERSONAS.find((item) => item.slug === slug)
  if (!persona) notFound()
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white px-4 sm:px-6 py-14 sm:py-20">
      <div className="max-w-3xl mx-auto">
        <p className="text-[11px] font-bold tracking-[0.16em] uppercase text-orange-400 mb-4">Outplacement buyer role</p>
        ...
        <div className="flex flex-wrap gap-3">
          <TrackLink
            ...
            logToUserEvents
            properties={{ channel: 'outplacement', cta_label: `Open ${persona.slug} destination`, source_page: `/outplacement/personas/${persona.slug}` }}
          ...
          >
            Continue to outplacement journey
          </TrackLink>
          <TrackLink
            href="/outplacement/personas"
            event={EVENT_NAMES.personaRouteSelected}
            logToUserEvents
            properties={{ channel: 'outplacement', persona: 'persona_back_nav', source_route: `/outplacement/personas/${persona.slug}`, target_route: '/outplacement/personas' }}
