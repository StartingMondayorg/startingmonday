...
export default function LiveBriefPublicPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (error) return <main className="flex min-h-screen items-center justify-center bg-background px-6"><div className="max-w-md text-center"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary">Starting Monday</p><p role="heading" aria-level={1} className="mt-4 text-3xl font-semibold text-foreground">This brief is no longer available</p><p className="mt-3 text-sm leading-6 text-muted-foreground">The private link may have expired or been revoked.</p></div></main>
    ...
    <main className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-foreground text-background">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-5">
          ...
          <span className="text-[11px] uppercase tracking-[0.14em] text-muted">Private brief</span>
        </div>
      </header>
      <div className="mx-auto max-w-4xl px-6 py-12 sm:py-16">
        ...
        <div className="mt-10 space-y-5">
          {sections.length > 0 ? sections.map((section, index) => {
            ...
            return <section key={`${name}-${index}`} onClick={() => void record('delivery_section_viewed', name)} className="border-t border-border py-6"><h2 className="text-xl font-semibold text-foreground">{name}</h2><div className="mt-3 whitespace-pre-wrap text-[15px] leading-7 text-foreground/80">{typeof content.content === 'string' ? content.content : JSON.stringify(content, null, 2)}</div></section>
          }) : <section className="border-t border-border py-6"><h2 className="text-xl font-semibold">{sectionTitle(title)}</h2><pre className="mt-3 whitespace-pre-wrap text-[15px] leading-7 text-foreground/80">{JSON.stringify(payload, null, 2)}</pre></section>}
        </div>

        <section className="mt-10 border-y border-border py-8"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-primary">Next step</p><h2 className="mt-3 text-2xl font-semibold">Go deeper with Rich</h2><p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Book a working session to pressure-test the strongest opportunities and decide what to do next.</p><a href={bookingUrl} onClick={() => void record('delivery_cta_clicked')} className="mt-6 inline-flex rounded bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Book a working session</a></section>
        <footer className="pt-8 text-[11px] leading-5 text-muted-foreground">Private by default. This private link is time-limited. Access is logged to help the sender understand whether the brief was received. Sources and limits are preserved in the brief artifact.</footer>
      </div>
    </main>
  )
}